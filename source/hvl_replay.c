...
{
  static const uint16 LengthTable[45] = { 3, 7, 0xf, 0x1f, 0x3f, 0x7f, 3, 7, 0xf, 0x1f, 0x3f, 0x7f,
...
struct hvl_tune *hvl_load_ahx( uint8 *buf, uint32 buflen, uint32 defstereo, uint32 freq )
{
  ...
  strncpy( ht->ht_Name, (TEXT *)&buf[(buf[4]<<8)|buf[5]], 80 );
  ht->ht_Name[79] = 0;
  ...
  for( i=1; i<=ht->ht_InstrumentNr; i++ )
  {
    ...
    {
      strncpy( ht->ht_Instruments[i].ins_Name, nptr, 32 );
	  ht->ht_Instruments[i].ins_Name[31] = 0;
      nptr += strlen( nptr )+1;
    } else {
...
struct hvl_tune *hvl_LoadTune( TEXT *name, uint32 freq, uint32 defstereo, int size )
{
  uint8  *buf;
  FILE *f;
  ...
  return hvl_reset( buf, size, defstereo, freq, 1 ); 
}
...
void hvl_set_audio( struct hvl_voice *voice, float64 freqf )
{
  ...
  if( voice->vc_NewWaveform )
  {
    ...
    if( voice->vc_Waveform == 4-1 )
    {
      ...
      memcpy( &voice->vc_VoiceBuffer[0], (void *)src, 0x280);
    } else {
      uint32 i, WaveLoops;
      ...
      for( i=0; i<WaveLoops; i++ )
        ...
        memcpy( &voice->vc_VoiceBuffer[i*4*(1<<voice->vc_WaveLength)],
                (void *)src,
                4*(1<<voice->vc_WaveLength));
    }
  ...
  if( voice->vc_RingNewWaveform )
  {
    ...
    for( i=0; i<WaveLoops; i++ )
     ...
     memcpy( &voice->vc_RingVoiceBuffer[i*4*(1<<voice->vc_WaveLength)],
              (void *)src,
              4*(1<<voice->vc_WaveLength));
